...
export default function Chatbox() {
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white shadow-xl rounded-2xl h-[550px] flex flex-col border border-gray-300 overflow-hidden">
      {/* Chat Messages */}
      <div className="flex-1 overflow-y-auto p-2 space-y-2 flex flex-col">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-500">
            ...
            <p className="text-sm">Start a conversation with your rider</p>
          </div>
        ) : (
          messages.map((msg, index) => (
            <motion.div
              ...
              className={`p-3 rounded-lg max-w-xs border ${
                msg.sender === "user"
                  ...
                  : "bg-gray-100 text-black self-start text-left border-gray-300 shadow-sm"
              }`}
              style={{
                borderRadius: "12px",
                padding: "10px 15px",
      ...
      <div className="grid grid-cols-2 gap-2 p-2">
        {predefinedMessages.map((msg, index) => (
          <button
            ...
            className="px-3 py-2 bg-gray-300 text-black rounded-lg hover:bg-gray-400 transition duration-200 text-sm shadow-sm"
          >
            {msg}
          </button>
        ))}
